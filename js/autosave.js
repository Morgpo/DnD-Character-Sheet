...
// Save the current sheet data to localStorage
function autoSaveSheet() {
    try {
        // Check if required functions exist
        if (typeof getAttacks === 'undefined' || 
            typeof getCharges === 'undefined' || 
            typeof getEquipment === 'undefined' || 
            typeof getSpells === 'undefined' || 
            typeof getSpellSlots === 'undefined') {
            console.error('Autosave: Required functions not yet loaded');
            return;
        }

        // Use the same data structure as saveSheet() from save.js
        var sheet = {
            page1: {
                basic_info: {
        ...
        localStorage.setItem('dnd_character_sheet_autosave', JSON.stringify(sheet));
        ...
        if (!window.autosaveCount) window.autosaveCount = 0;
        window.autosaveCount++;
        if (window.autosaveCount % 60 === 0) { // Log every 60 saves (every minute)
            console.log('Autosave: Successfully saved (count: ' + window.autosaveCount + ')');
        }
...
// Initialize autosave when document is ready
$(document).ready(function() {
    ...
    setTimeout(function() {
        ...
        if (typeof getAttacks === 'undefined' || 
            ...
            typeof getSpells === 'undefined' || 
            typeof getSpellSlots === 'undefined') {
            console.error('Autosave: Required functions not available. Check script load order.');
            return;
        }
        
        console.log('Autosave: All required functions loaded');
        
        // Start autosaving every 1 second
        setInterval(autoSaveSheet, 1000);
        
        // Do initial save
        autoSaveSheet();
        
        console.log('Autosave: Started - saving every 1 second');
    }, 500);
});
